...
public class TicketDAO {
    ...
    public boolean saveTicket(Ticket ticket){
        ...
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement(DBConstants.SAVE_TICKET)) {
				...
				ps.setTimestamp(5, (ticket.getOutTime() == null)?null: (new Timestamp(ticket.getOutTime())) );
				return ps.execute();
			}
        }catch (Exception ex){
    ...
    public int getNumberOccurence(String vehicleRegNumber) {
        ...
        try {
            con = dataBaseConfig.getConnection();
            try (PreparedStatement ps = con.prepareStatement(DBConstants.GET_NUMBER_OCCURRENCE)) {
				//ID, PARKING_NUMBER, VEHICLE_REG_NUMBER, PRICE, IN_TIME, OUT_TIME)
				ps.setString(1,vehicleRegNumber);
				ResultSet rs = ps.executeQuery();
				if(rs.next()){
